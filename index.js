...
function main() {
  if (!process.env.TIVO_HOSTS) {
    throw new Error("ENV TIVO_HOSTS is required");
  }
  const config = process.env.TIVO_HOSTS.split(",");
  for (const cfg of config) {
    const [device, ip] = cfg.split(":");
    tivos[device] = new TiVoHost({ device: device, ip: ip });
  }
}
